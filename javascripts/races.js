...
function candidateLister(d) {
	...
	for (i=0;i<data.length;i++){
		...
		// Seat ID
		if (seat.indexOf("Senator") === 6 || seat.indexOf("Representative") === 6) {
			seatID = ("#mn" + (seat.substring(seat.indexOf("District") + 9))).toLowerCase();
			console.log(seatID);
		} else if (seat.indexOf("Representative") === 5) {
			seatID = "#us" + (seat.substring(seat.indexOf("District") + 9));
			console.log(seatID);
		} else {
			seatID = "#us";
			console.log(seatID);
